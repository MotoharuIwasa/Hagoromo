...
namespace Hagoromo.DevelopableMesh
{
    ...
    //エッジ長さ保存エネルギー (E = 0.5 * w * (L - L0)^2)、等式制約条件: L - L0 = 0
    public class EdgeLengthEnergy2D2 : EnergyTerm
    {
        //------------------------------GetEnergyなどの関数でx[]以外に使いたい変数をここに書いておく。
        private List<int[]> edges;
        private double[] initialLength;

        public EdgeLengthEnergy2D2(double weight, List<int[]> edges, double[] initialLength) : base(weight)
        {
            //重複しているエッジは取り除いたエッジのリスト
            this.edges = edges;
            this.initialLength = initialLength;
        ...
        public override double GetEnergy(double[] x)
        {
            ...
            for (int i = 0; i < edges.Count; i++)
            {
                ...
                double diff = len - initialLength[i];
                E += 2 * Weight * Math.Sqrt(Math.Abs(diff));
            }
            return E;
        }
        ...
        public override void AddGradient(double[] x, double[] g)
        {
            for (int i = 0; i < edges.Count; i++)
            {
                ...
                double diff = len - initialLength[i];
                double factor = 0;
                if(diff> 0) { factor = Weight / (Math.Sqrt(Math.Abs(diff)) * len); }
                else { factor = -Weight / (Math.Sqrt(Math.Abs(diff)) * len); }
