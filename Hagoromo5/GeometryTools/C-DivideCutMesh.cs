...
namespace Hagoromo.GeometryTools
{
    public class DivideCutMesh : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            //List<CutMesh> cutMeshes = SplitIntoConnectedComponents(cutMesh);
            List<MeshSplitResult> cutMeshes2 = SplitIntoConnectedComponents2(cutMesh);
            List<CutMesh> cutMeshes = new List<CutMesh>();
            foreach (MeshSplitResult result in cutMeshes2) { cutMeshes.Add(result.Mesh); }
            List<GH_CutMesh> ghCutMeshes = new List<GH_CutMesh>();
            for (int i =0; i <cutMeshes.Count; i++)
            {
                ghCutMeshes.Add(new GH_CutMesh(cutMeshes[i].Sort2()));
            }
