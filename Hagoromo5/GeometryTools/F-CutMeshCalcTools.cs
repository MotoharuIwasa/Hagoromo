...
namespace Hagoromo.GeometryTools
{
    public static class CutMeshCalcTools
    {
        ...
        public static double[] GaussianMap(CutMesh mesh, int iterations)
        {
            double[] preGaussian = GaussianCurvature(mesh);
            List<int> boundaryVertIndices = mesh.BoundaryVertIndices();
            ...
            HashSet<int> internalVert = internalVertIndices.ToHashSet();
            double[] currentGaussian = new double[preGaussian.Length];
            for (int k = 0; k < iterations; k++)
            {
                foreach (int i in internalVertIndices)
                {
                    List<int> connected = mesh.GetVerticesForVertex(i);
                    int j = 0;
                    double gauss = 0;
                    foreach (int vert in connected)
                    {
                        if (internalVert.Contains(vert)) { j += 1; gauss += preGaussian[vert]; }
                    }
                    if (j != 0) { gauss /= j; }
                    gauss = (gauss + preGaussian[i]) * 0.5;
                    currentGaussian[i] = gauss;
                }
                preGaussian = currentGaussian;
            }
            return currentGaussian;
        }

        //-----------------------------------------------------------grouping----------------------------------------------------------------------------------------
        public static List<List<int>> FindSeparatedPeaksTwoPass(CutMesh cutMesh, List<double> gaussMap, double threshold)
        {
            cutMesh.ReloadVertexToEdgesCache();
            int vertexCount = cutMesh.Vertices.Count;

            // 全グループを格納するリスト
            List<List<int>> allClusters = new List<List<int>>();
            // すでにいずれかのパスで割り当てられたかを確認する
            bool[] assigned = new bool[vertexCount];

            // --- Pass 1: Positive Peaks (山) ---
            allClusters.AddRange(ProcessWatershed(cutMesh, gaussMap, threshold, assigned, true));

            // --- Pass 2: Negative Peaks (谷) ---
            // 負のパスでは threshold をマイナスにして判定
            allClusters.AddRange(ProcessWatershed(cutMesh, gaussMap, threshold, assigned, false));

            for (int i = allClusters.Count - 1; i >=0; i--)
            {
                List<int> cluster = allClusters[i];
                if (cluster.Count < cutMesh.Vertices.Count*0.01) { allClusters.RemoveAt(i); continue; }
                double total = 0;
                foreach (int vert in cluster)
                {
                    total += Math.Abs(gaussMap[vert]);
                }
                if (total < 0.05) { allClusters.RemoveAt(i); }
            }

            return allClusters;
        }

        private static List<List<int>> ProcessWatershed(CutMesh cutMesh, List<double> gaussMap, double threshold, bool[] globalAssigned, bool isPositivePass)
        {
            int vertexCount = cutMesh.Vertices.Count;
            int[] peakOwner = new int[vertexCount];
            for (int i = 0; i < vertexCount; i++) peakOwner[i] = -1;

            // 1. シード（極値点）の特定
            for (int i = 0; i < vertexCount; i++)
            {
                if (globalAssigned[i]) continue;

                double val = gaussMap[i];
                if (isPositivePass)
                {
                    if (val < threshold) continue; // 正の閾値未満は無視
                    if (IsLocalExtreme(cutMesh, i, gaussMap, true)) peakOwner[i] = i;
                }
                else
                {
                    if (val > -threshold) continue; // 負の閾値(例:-0.01)より大きい(例:-0.005)は無視
                    if (IsLocalExtreme(cutMesh, i, gaussMap, false)) peakOwner[i] = i;
                }
            }

            // 2. 勾配を辿って各頂点をシードに割り当てる
            for (int i = 0; i < vertexCount; i++)
            {
                if (globalAssigned[i] || peakOwner[i] != -1) continue;

                // 閾値判定
                if (isPositivePass && gaussMap[i] < threshold) continue;
                if (!isPositivePass && gaussMap[i] > -threshold) continue;

                int current = i;
                List<int> path = new List<int> { current };

                while (peakOwner[current] < 0)
                {
                    int next = FindSteepestNeighbor(cutMesh, current, gaussMap, isPositivePass);
                    if (next == -1 || next == current) break;

                    current = next;
                    path.Add(current);
                    if (peakOwner[current] >= 0) break;
                }

                if (peakOwner[current] >= 0)
                {
                    int finalOwner = peakOwner[current];
                    foreach (int p in path)
                    {
                        peakOwner[p] = finalOwner;
                        globalAssigned[p] = true; // 他のパスで使われないようにロック
                    }
                }
            }
            ...
            for (int i = 0; i < vertexCount; i++)
            {
                if (peakOwner[i] >= 0)
                {
                    if (!groups.ContainsKey(peakOwner[i])) groups[peakOwner[i]] = new List<int>();
                    groups[peakOwner[i]].Add(i);
                }
            }

            // 頂上/谷底を先頭にして返す
            return groups.Values.Select(g =>
                isPositivePass
                ? g.OrderByDescending(idx => gaussMap[idx]).ToList()
                : g.OrderBy(idx => gaussMap[idx]).ToList()
            ).ToList();
        }

        public static bool IsLocalExtreme(CutMesh mesh, int idx, List<double> map, bool findMax)
        {
            double val = map[idx];
            foreach (int n in mesh.GetVerticesForVertex(idx))
            {
                if (findMax) { if (map[n] > val) return false; }
                else { if (map[n] < val) return false; }
            }
            return true;
        }

        public static int FindSteepestNeighbor(CutMesh mesh, int idx, List<double> map, bool findUp)
        {
            int bestIdx = -1;
            double bestVal = map[idx];
            foreach (int n in mesh.GetVerticesForVertex(idx))
            {
                if (findUp)
                {
                    if (map[n] > bestVal) { bestVal = map[n]; bestIdx = n; }
                }
                else
                {
                    if (map[n] < bestVal) { bestVal = map[n]; bestIdx = n; }
        ...
        public static int FindCentralVertexIndex(CutMesh cutMesh, List<int> cluster)
        {
            ...
            foreach (int vIdx in cluster)
            {
                centroid += cutMesh.Vertices[vIdx];
            }
            centroid /= cluster.Count;

            // 2. 重心に最も近い頂点をクラスター内から探す
            int centralIndex = -1;
            double minDistanceSq = double.MaxValue;

            foreach (int vIdx in cluster)
            {
                Point3d vPos = cutMesh.Vertices[vIdx];
                // 距離の比較なので、計算コストの低い「距離の2乗」を使用
                double distSq = centroid.DistanceToSquared(vPos);
        ...
        public static (List<int> OuterVerts, List<int> OuterEdges) FindOuterBoundaryVerts(CutMesh cutMesh)
        {
            ...
            */
            List<int> longestVerts = boundaryVertsGroup.OrderByDescending(l => l.Count).First();
            List<int> longestEdges = boundaryGroup.OrderByDescending(l => l.Count).First();
            return (longestVerts, longestEdges);
            //return outerVerts;
        }
        ...
        //エッジの順にその長さをまとめた配列
        public static double[] EdgeLength(CutMesh mesh)
        {
            ...
            {
                int[] e = mesh.Edges[i];
                length[i] = mesh.Vertices[e[0]].DistanceTo(mesh.Vertices[e[1]]);
            }
            return length;
        }
        ...
        //２次元上の話。符号付面積が正になるような方でfaceの三点の順番を決定する。
        //(x1-x0)(y2-y0)-(x2-x0)(y1-y0)で計算する。
        public static int[,] FacesWithOrder(CutMesh mesh)
        {
            List<List<int>> faceNormalOrders = FaceNormalOrders(mesh);
            List<List<int>> faceRecord = faceNormalOrders.OrderBy(subList => subList[0]).ToList();

            int[] edge1 = mesh.Edges[faceRecord[0][1]];
            int[] edge2 = mesh.Edges[faceRecord[0][2]];
            ...
            int faceCount = mesh.Faces.GetLength(0);
            int[,] facesWithOrder = new int[faceCount, 3];
            if (area > 0)
            {
                for (int i = 0; i < faceCount; i++)
                {
                    edge1 = mesh.Edges[faceRecord[i][1]];
                    edge2 = mesh.Edges[faceRecord[i][2]];
                    v0index = edge1.Intersect(edge2).First();
                    facesWithOrder[i, 0] = v0index;
                    facesWithOrder[i, 1] = edge1.First(f => f != v0index);
                    facesWithOrder[i, 2] = edge2.First(f => f != v0index);
                }
            }
            else
            {
                for (int i = 0; i < faceCount; i++)
                {
                    edge1 = mesh.Edges[faceRecord[i][2]];
                    edge2 = mesh.Edges[faceRecord[i][1]];
                    v0index = edge1.Intersect(edge2).First();
                    facesWithOrder[i, 0] = v0index;
                    facesWithOrder[i, 1] = edge1.First(f => f != v0index);
                    facesWithOrder[i, 2] = edge2.First(f => f != v0index);
        ...
        //メッシュのエッジに沿ったcrv(polylineのままでOK)を与えられたときにそのcurveがどのedge indicesに対応するか返す関数
        public static List<int> CrvToEdgeIndices(CutMesh cutMesh, List<Curve> curves)
        {
        ...
        //メッシュのエッジに沿ったpolylineの点のdupにおけるインデックスを順番に返す。clsedなら最後の点→最初の点...→最後の点→最初の点としている。
        public static List<List<int>> CrvToDupVertIndices(CutMesh cutMesh, List<Curve> curves)
        {
            ...
            foreach (Polyline polyline in cutLines)
            {
                ...
                foreach (Point3d pt in polyline)
                {
                    ...
                    topoVertsOnPolyline.Add(vertOrderInDup[closestTv]);
                }
                if (polyline.IsClosed)
                {
                    if (topoVertsOnPolyline.Count > 2)
                    {
                        int first = topoVertsOnPolyline[0];
                        int last = topoVertsOnPolyline[topoVertsOnPolyline.Count - 1];

                        topoVertsOnPolyline.Insert(0, last);   // 最後の要素を先頭に
                        topoVertsOnPolyline.Add(first);        // 元の先頭を末尾に
                    }
                }
                if (topoVertsOnPolyline.Count > 2) { vertIndices.Add(topoVertsOnPolyline); }
            }
            return vertIndices;
        }

        //メッシュのエッジに沿ったcrv(polylineのままでOK)を与えられたときにそのcurveがどのvertIndicesに対応するか返す関数
        //重複する点も全てリストアップして返す。
        public static List<int> CrvToVertIndices(CutMesh cutMesh, List<Curve> curves)
        {
...
;
            ...
            foreach (Point3d Pt in Points)
            {
                ...
                {
                    double dist = Pt.DistanceToSquared(vertices[i]);
                    if (dist < minDist)
                    {
                        minDist = dist;
                        closestIndex = i;
                ...
                if (closestIndex != -1)
                    vertIndices.Add(closestIndex);
            }
            List<int> ListVertIndices = vertIndices.ToList();
            HashSet<int> expandedSet2 = new HashSet<int>(ListVertIndices);
            foreach (int idx in ListVertIndices)
            {
            ...
            List<int> sortedVertIndices = expandedSet2.ToList();
            sortedVertIndices.Sort();
            return sortedVertIndices;
        }
    }
}