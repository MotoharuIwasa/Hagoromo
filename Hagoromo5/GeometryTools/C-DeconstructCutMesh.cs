...
namespace Hagoromo.GeometryTools
{
    public class DeconstructCutMesh : GH_Component
    {
        ...
        protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager)
        {
            ...
            pManager.AddIntegerParameter("Dups", "D", "dups", GH_ParamAccess.tree);
        }
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            List<List<int>> dups = cutMesh.DuplicatedVertIndices;
            var tree3 = new GH_Structure<GH_Integer>();

            for (int i = 0; i < dups.Count; i++)
            {
                GH_Path path3 = new GH_Path(i); // edgeごとにPathを作成
                foreach (int v in dups[i])
                {
                    tree3.Append(new GH_Integer(v), path3);
                }
            }
            DA.SetDataTree(3, tree3);
