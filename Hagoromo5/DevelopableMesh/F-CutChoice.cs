    ...
    public static class CutChoiceTools
    {
        ...
        */
        ...
        public static double[] TotalLengthAndObj(CutMesh mesh, List<List<int>> path, List<double> edgeLength, bool[] isCut)
        {
            ...
            int totalFaceCount = mesh.Faces.GetLength(0);
            double minFaceCount = totalFaceCount * 0.01;
            
            CutMesh cutMesh2 = CutMeshWithEdgeIndices(mesh, edges.ToList());
            int branchPenalty = cutMesh2.DuplicatedVertIndices.Count(sub => sub.Count >= 3);
            ...
            {
                if (cutMesh.Faces.GetLength(0) < minFaceCount) { return new double[] { double.MaxValue, double.MaxValue, double.MaxValue }; }
                loopCount += 1;
                List<int> outerBoundary = FindOuterBoundaryVerts(cutMesh).OuterVerts;
                List<List<List<int>>> category = CategolizeCutMesh(cutMesh, false, false, false, outerBoundary, null);
                ...
                loopCount += Fblocks.Count + DtwoMirror.Count + DoneMirror.Count;
                ...
                double[] angleSum = AngleSum(cutMesh);
                var loopData = CalcLoopData(cutMesh, angleSum, DoneMirror, Fblocks);
                ...
                foreach (int[] loop in Fblocks2)
                {
                    ...
                    n = loopCos.Count;
                    double value = w3  * Math.PI / aveLength;
            ...
            double seamPenalty = branchPenalty * 0.5 + loopCount -15;
            //double seamPenalty = energy;
            return new double[] {totalLength, energy, totalLength };
        }

        public static double[] TotalLengthAndObj2(CutMesh mesh, List<List<int>> path, List<double> edgeLength, int[,] negPairIndicesInIsCut, bool[] isCut)
        {
            double totalLength = 0;
            HashSet<int> edges = new HashSet<int>();
            for (int i = 0; i < isCut.Length; i++)
            ...
            for (int i = 0; i < negPairIndicesInIsCut.GetLength(0); i++)
            {
                int negStartCheck = 0;
                for (int j = 0; j < negPairIndicesInIsCut.GetLength(1); j++) { if (isCut[negPairIndicesInIsCut[i, j]]) { negStartCheck++; } }
                if (negStartCheck == 1) { negStart += 1; }
            }
            ...
            foreach (CutMesh cutMesh in cutMeshes)
            {
                if (cutMesh.Faces.GetLength(0) < minFaceCount) { negStart += 5; }
                loopCount += 1;
                List<int> outerBoundary = FindOuterBoundaryVerts(cutMesh).OuterVerts;
                List<List<List<int>>> category = CategolizeCutMesh(cutMesh, false, false, false, outerBoundary, null);
            ...
            return new double[] { totalLength, energy, negStart };
        }
    }
}
