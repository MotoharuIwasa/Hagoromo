...
namespace Hagoromo.DevelopableMesh
{
    public class CutByTear : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            int sortedLength = sortedOuterVertIndices.Count;
            ...
            for (int i = 0; i < iter; i++)
            {
                ...
                List<BoundaryLoop> loops = mesh2d.GetBoundaryLoops();
                List<int> cutEdges = FindNextCutEdge(mesh3d, mesh2d, loops);
                if (cutEdges == null) { break; }
                mesh3d = CutMeshWithEdgeIndices(mesh3d, cutEdges).Sort();
                loops = mesh3d.GetBoundaryLoops();
                int max = 0;
                foreach (BoundaryLoop loop in loops)
                {
                    int count = loop.VertexIndices.Count;
                    if (count > max) { max = count; }
                }
                sortedOuterVertIndices = Enumerable.Range(0, max).ToList();
                mesh3dHistory.Add(new GH_CutMesh(mesh3d.Clone()));
            }
