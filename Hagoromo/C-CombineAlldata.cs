...
namespace Hagoromo.DataStructure
{
    public class CombineAlldata : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (!DA.GetData(5, ref gravity_vec))
            {
            ...
            var iconLookup = new Dictionary<Point3d, bool[]>();
            for (int i = 0; i < icondata.NodeArray.GetLength(0); i++)
            {
                ...
                // Icon Lookup
                var iconRow = new bool[6];

                for (int j = 0; j < 6; j++)
                {
                    var iconData = (bool[])icondata.IconArray[i, 0];
                    if (iconData[j] is bool value)
                    {
                        iconRow[j] = value;
                    }
                    else
                    {
                        throw new InvalidCastException("IconArray element is not of type bool.");
                    }
                }
            ...
            for (int i = 0; i < nodeCount; i++)
            {
                ...
                var iconRow = new bool[6];
                if (iconLookup.TryGetValue(new Point3d(position[0], position[1], position[2]), out bool[] iconValues))
                {
            ...
            double[] gravityArray = new double[3] { gravity_vec.X, gravity_vec.Y, gravity_vec.Z };
            ...
            for (int i = 0; i < nodeCount; i++)
            {
                ...
                var icon = string.Join(",", (bool[])node1);
                ...
                var row = new List<string>
                    {
                        ...
                        $"[{string.Join(",", (bool[])elemdata.ElementArray[i, 2])}]",
